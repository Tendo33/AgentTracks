...
class TestMainEndpoints:
    ...
    async def test_available_agents_endpoint(self, async_client):
        ...
        mock_langgraph_json = {
            "graphs": {"agent1": {}, "agent2": {}, "react_agent": {}}
        }

        with (
            patch("builtins.open", mock_open_html("../langgraph.json")),
...
class TestChatMessage:
    ...
    @pytest.mark.asyncio
    async def test_chat_message_without_thread_id(
        self, async_client, mock_build_workflow
    ):
        ...
        with patch("builtins.open", mock_open_html("../page.html")):
            chat_data = {"message": "Hello without thread_id"}

            response = await async_client.post("/chat-message", json=chat_data)
            assert response.status_code == 200

    @pytest.mark.asyncio
    async def test_chat_message_streaming_response(
        self, async_client, mock_build_workflow
    ):
...
class TestChatMessageModel:
    ...
    def test_chat_message_creation(self):
        ...
        message = ChatMessage(
            message="Test message", thread_id="test_thread", agent="test_agent"
        )
...
def mock_open_html(filename):
    ...
    def mock_open(*args, **kwargs):
        ...
        mock_file.read.return_value = (
            f"<html><body><h1>Mock {filename}</h1></body></html>"
        )
