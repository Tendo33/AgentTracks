...
shopping_list_agent = Agent(
    ...
    model=OpenAIChat(id="gpt-4o-mini", api_key=api_key, base_url=base_url),
...
shopping_team.print_response("I got bread", stream=True, stream_intermediate_steps=True)
print(f"Session state: {shopping_team.team_session_state}")

shopping_team.print_response(
    "I need apples and oranges", stream=True, stream_intermediate_steps=True
)
print(f"Session state: {shopping_team.team_session_state}")
