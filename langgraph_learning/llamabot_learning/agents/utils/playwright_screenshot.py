...
async def capture_page_and_img_src(url: str, image_path: str) -> tuple[str, list[str]]:
    if not PLAYWRIGHT_AVAILABLE:
        print(
            "Warning: Playwright is not installed. Screenshot functionality is disabled."
        )
        print(
            "To enable screenshots, install playwright: pip install playwright && playwright install"
        )
        return "", []
...
if __name__ == "__main__":
    trimmed_html, image_sources = asyncio.run(
        capture_page_and_img_src(
            "https://www.google.com", "assets/google-screenshot.png"
        )
    )
    print(trimmed_html)
    print(image_sources)
